        ...
        while (ctx->in) {
            ...
            if (ctx->buf == NULL) {
                ...
                if (rc != NGX_OK) {
                    ...
                    } else if (ngx_output_chain_get_buf(ctx, bsize) != NGX_OK) { // 分配ctx->b: 一个32KB的buffer
                        return NGX_ERROR;
                    }
                }
            }
            ...
            cl = ngx_alloc_chain_link(ctx->pool); // 虽然是一个32KB的大buffer 也要分割成小chain 并挂到out上
            if (cl == NULL) {
                return NGX_ERROR;
            }
...
static ngx_inline ngx_int_t
ngx_output_chain_as_is(ngx_output_chain_ctx_t *ctx, ngx_buf_t *buf) // 反1不用拷贝(as is) 反0需拷贝
{
    ngx_uint_t  sendfile;
