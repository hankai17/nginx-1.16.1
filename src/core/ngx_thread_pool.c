...
ngx_thread_pool_cycle(void *data)
{
    ...
    for ( ;; ) {
        ...
        task->handler(task->ctx, tp->log);          // 1.执行回调
        ...
        *ngx_thread_pool_done.last = task;
        ...
        ngx_unlock(&ngx_thread_pool_done_lock);
                                                    ...
                                                    // 2.执行回调结果(异步)
        (void) ngx_notify(ngx_thread_pool_handler); // 把ngx_thread_pool_handler 初始化到e的data泛指针上
                                                    // 然后在 notify_fd的回调(ngx_epoll_notify_handler)里 拿到泛指针开始回调ngx_thread_pool_handler
    }
}
