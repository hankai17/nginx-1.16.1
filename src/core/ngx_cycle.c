...
ngx_init_cycle(ngx_cycle_t *old_cycle)              // 起各worker之前 // reload时
{
    ...
    for (i = 0; cycle->modules[i]; i++) {       // 配置文件之1提前布core模块得局
    ...
    for (i = 0; cycle->modules[i]; i++) {
        ...
        if (module->init_conf) {
            if (module->init_conf(cycle,
                                  cycle->conf_ctx[cycle->modules[i]->index]) // 配置文件之3 解析完整个配置后 init core module: ngx_event_init_conf
                == NGX_CONF_ERROR)
            {
                environ = senv;
                ngx_destroy_cycle_pools(&conf);
    ...
    for (i = 0; /* void */ ; i++) { // 在main中起各模块设置得共享内存
    ...
    if (ngx_init_modules(cycle) != NGX_OK) { // eg: 分配全局共享内存 eg:记录各种参数ngx_stat_accepted
        /* fatal */
        exit(1);
    }
