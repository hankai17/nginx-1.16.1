...
ngx_set_inherited_sockets(ngx_cycle_t *cycle)
{
    ...
    for (i = 0; i < cycle->listening.nelts; i++) {
        ...
        olen = sizeof(int);
        if ( getsockopt(ls[i].fd, SOL_IP, IP_TRANSPARENT,
                    (void *) &ls[i].transparent, &olen)
                == -1)  {
            err = ngx_socket_errno;
            ngx_log_error(NGX_LOG_NOTICE, cycle->log, err, 
...
#endif
            ngx_log_debug1(NGX_LOG_DEBUG_CORE, log, 0, "ls[i].transparent %d", ls[i].transparent);
            if (ls[i].transparent == 1) {
                if (setsockopt(s, SOL_IP, IP_TRANSPARENT, (const void *) &ls[i].transparent, sizeof(int)) == -1) {
                    ngx_log_error(NGX_LOG_EMERG, log, ngx_socket_errno,
                            "setsockopt(IP_TRANSPARENT) %V failed, ignored",
                            &ls[i].addr_text);
                    if (ngx_close_socket(s) == -1) {
                        ngx_log_error(NGX_LOG_EMERG, log, ngx_socket_errno,
                                ngx_close_socket_n " %V failed",
    ...
    for (i = 0; i < cycle->listening.nelts; i++) {
        ...
        if (ls[i].transparent == 1) {
            if (setsockopt(ls[i].fd, SOL_IP, IP_TRANSPARENT,
                        (const void *) &ls[i].transparent, sizeof(int))
                    == -1)  {
                ngx_log_error(NGX_LOG_ALERT, cycle->log, ngx_socket_errno,
                        "setsockopt(IP_TRANSPARENT, %d) %V failed, ignored",
                        ls[i].transparent, &ls[i].addr_text);
            }
        }
...
ngx_connection_t *
ngx_get_connection(ngx_socket_t s, ngx_log_t *log) // 拿到一个空闲的c 初始化fd 并反转c->r/wev的instance
{
    ngx_uint_t         instance;
    ngx_event_t       *rev, *wev;
    ngx_connection_t  *c;
...
void
ngx_reusable_connection(ngx_connection_t *c, ngx_uint_t reusable) // reusable 用于长连接
{
