...
ngx_http_proxy_handler(ngx_http_request_t *r) // upstream 是在request的handler中 开始的 // ngx_http_core_module.c +1289 // 0UPSTREAM
{
    ...
    u->buffering = plcf->upstream.buffering; // os得buffer
...
static ngx_int_t
ngx_http_proxy_process_header(ngx_http_request_t *r) // 1解析os响应头
{
    ngx_int_t                       rc;
    ngx_table_elt_t                *h;
    ngx_http_upstream_t            *u;
...
static ngx_int_t
ngx_http_proxy_copy_filter(ngx_event_pipe_t *p, ngx_buf_t *buf) // 深拷贝
{
    ngx_buf_t           *b;
    ngx_chain_t         *cl;
    ...
    cl = ngx_chain_get_free_buf(p->pool, &p->free); // 深拷贝
    if (cl == NULL) {
        return NGX_ERROR;
    }
...
ngx_http_proxy_merge_loc_conf(ngx_conf_t *cf, void *parent, void *child)
{
    ...
    {
        clcf->handler = ngx_http_proxy_handler; // upstream 是在request的handler中 开始的 // ngx_http_core_module.c +1289
    }
...
ngx_http_proxy_pass(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    clcf = ngx_http_conf_get_module_loc_conf(cf, ngx_http_core_module);     // 拿到当前块(cf->ctx是location级别的3*50[]数组集合)的location数组中的core_module 即数组中的第一个

    clcf->handler = ngx_http_proxy_handler;                                 // pass关键字的作用是 设置clcf的回调
