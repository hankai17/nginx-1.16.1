...
ngx_module_t  ngx_http_sub_filter_module = { // 1.定义全局的module(ngx_modules.c)
    NGX_MODULE_V1,
    &ngx_http_sub_filter_module_ctx,       /* module context */ // 3. 设置该module的全局ctx 用于提前布局跟hook注册
    ngx_http_sub_filter_commands,          /* module directives */
    NGX_HTTP_MODULE,                       /* module type */ // 2.设置为http类型
    NULL,                                  /* init master */
    NULL,                                  /* init module */
    NULL,                                  /* init process */
    NULL,                                  /* init thread */
...
ngx_http_sub_body_filter(ngx_http_request_t *r, ngx_chain_t *in)
{
    ...
    if (in) {
        if (ngx_chain_add_copy(r->pool, &ctx->in, in) != NGX_OK) { // 为何重新分配chain? 直接用in不行吗? // 浅拷贝 只分配了chain 而公用了b  // 下面的切割形成的深拷贝块均引用该b
            return NGX_ERROR;
        }
    }
    ...
    while (ctx->in || ctx->buf) {
        ...
        while (ctx->pos < ctx->buf->last) {
            ...
            ngx_log_debug4(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                           ...
                           rc, &ctx->looked, ctx->copy_start, ctx->copy_end); // rc0为成功 
            ...
            if (ctx->saved.len) {
                ...
                cl = ngx_chain_get_free_buf(r->pool, &ctx->free); // 深拷贝
                if (cl == NULL) {
                    return NGX_ERROR;
                }
            ...
            if (ctx->copy_start != ctx->copy_end) {// 没有匹配上  //"cccabcdefghijkldefab" copy_start指向开头 copy_end指向abc... // defghijkldefab start指向开头 end指向ab

                cl = ngx_chain_get_free_buf(r->pool, &ctx->free); // 深拷贝 // 需要割裂一个独立的buf
                if (cl == NULL) {
                    return NGX_ERROR;
                }
                ...
                *ctx->last_out = cl;
                ctx->last_out = &cl->next;  // 挂ctx->out上
            }
            ...
            /* rc == NGX_OK */ // 匹配上了, ctx->matches为配置文件中所有替换命令 ctx->index为匹配到了第几个命令

            cl = ngx_chain_get_free_buf(r->pool, &ctx->free);   // 深拷贝 分配新buf
            if (cl == NULL) {
                return NGX_ERROR;
            }
            ...
            if (sub->len) {
                b->memory = 1;      // 新buf指向 匹配到的那个ctx->matches
                b->pos = sub->data;
        ...
        // 匹配到结尾了                     // ctx->buf是 当前整个buf: cccabcdefghijkldefab // ctx->looked是 ab
        if (ctx->looked.len
            && (ctx->buf->last_buf || ctx->buf->last_in_chain)) // last_buf肯定不是1 因为os_threads sleep了
        {
        ...
        if (ctx->buf->last_buf || ctx->buf->flush || ctx->buf->sync // flush 为1 // 最后一个块在sleep后终于到来
            || ngx_buf_in_memory(ctx->buf))
        {
            ...
            b->last_buf = ctx->buf->last_buf;       // 此时b指向的是394行的 // defghijkldefab start指向开头 end指向ab
            b->last_in_chain = ctx->buf->last_in_chain;
            b->flush = ctx->buf->flush;
            b->shadow = ctx->buf;

            b->recycled = ctx->buf->recycled; // 此时: chain->chian->chian // 看下面
        }
...
ngx_http_sub_output(ngx_http_request_t *r, ngx_http_sub_ctx_t *ctx)
{
    ...
    rc = ngx_http_next_body_filter(r, ctx->out); // 将ctx->out 三个深拷贝串发出去了

    if (ctx->busy == NULL) {                    // 为何不用 update_chains ?
        ctx->busy = ctx->out;
    ...
    while (ctx->busy) {
        ...
        if (b->shadow) {
            b->shadow->pos = b->shadow->last; // 因为update_chains没有shadow处理 // 即假设有out有3个chain 如果底层发送时 仅发送1.5个 即没有把最后一个带有shadow的发出去 那么最开头传进来的in(ngx_http_sub_body_filter(ngx_http_request_t *r, ngx_chain_t *in)) 的buf不会有任何变化
        }
    ...
    if (ctx->in || ctx->buf) {
        ...
        r->buffered &= ~NGX_HTTP_SUB_BUFFERED; // 这里
    }

    //   我的疑问就是 ab最终时 sub_filter模块吞掉了? 如何验证?
...
/*
pos = 0x7c9982 "cccabcdefghijkldefab",
  last = 0x7c9985 "abcdefghijkldefab",
  file_pos = 0,
  file_last = 0,
  start = 0x0,
  end = 0x0,
  tag = 0x772b40 <ngx_http_proxy_module>,
  file = 0x0,
  shadow = 0x0,         --------------------------> 这一块不置shadow
  temporary = 0,
  memory = 1,
  mmap = 0,
  recycled = 0,
 ...
 pos = 0x7e2acf "111",
  ...
  tag = 0x772b40 <ngx_http_proxy_module>,
  file = 0x0,
  shadow = 0x7c9668,   ------------------------> 为何最后一块置shadown ?
  temporary = 0,
  memory = 1,
  mmap = 0,
  recycled = 0,
  ...
  num = 0
----------------------------------------------------------------------------------
ngx_http_next_body_filter(r, ctx->out);       底层将这三个块发送出去之后

 pos = 0x7c9985 "abcdefghijkldefab",
  last = 0x7c9985 "abcdefghijkldefab",
  file_pos = 0,
  file_last = 0,
  start = 0x0,
...
b->shadow->pos = b->shadow->last;  后
  ...
  temp_file = 0,
  num = 0
即使返回到ngx_http_upstream_process_non_buffered_request ngx_chain_update_chains时 是上面这个 pos==last 即代表可以清空



ngx_http_sub_parse仍然看不懂
