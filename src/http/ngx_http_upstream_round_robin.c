...
ngx_int_t
ngx_http_upstream_init_round_robin(ngx_conf_t *cf, // 调用时机是init main configuration中 ngx_http_upstream_init_main_conf // 即如果没有自定义的peer_init_stream则调用此
    ...
    ngx_http_upstream_srv_conf_t *us)
{
    ...
    rrp->peers = us->peer.data; // 将配置文件中的的proxy_pass地址 传给rrp
    rrp->current = NULL;
    rrp->config = 0;
    ...
    ngx_http_upstream_resolved_t *ur)
{
    ...
    r->upstream->peer.get = ngx_http_upstream_get_round_robin_peer; // get free是针对 一个upstream中有多个地址这种纯反向代理
    r->upstream->peer.free = ngx_http_upstream_free_round_robin_peer;
    r->upstream->peer.tries = ngx_http_upstream_tries(rrp->peers);
#if (NGX_HTTP_SSL)
...
void
ngx_http_upstream_free_round_robin_peer(ngx_peer_connection_t *pc, void *data, // ngx_http_upstream_finalize_request中调用
    ngx_uint_t state)
{
    ngx_http_upstream_rr_peer_data_t  *rrp = data;
