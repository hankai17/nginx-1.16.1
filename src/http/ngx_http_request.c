...
void
ngx_http_init_connection(ngx_connection_t *c) // 0pre事件可重入 协议分发?
{
    ngx_uint_t              i;
    ngx_event_t            *rev;
    ...
    if (port->naddrs > 1) {     // eg: 两个ip地址(127.0.0.1 | 10.53.80.152)监听同一端口80
                                //      可以根据客户端请求的目的ip可以知道客户端请求的是哪个server块(cscf)
                                //      参考ngx_http.c ngx_http_add_listening注释: addr里封装的有配置该<addr:port>所属的server块结构: cscf
                                //      -------------------> fd -> c -> hc 跟 cscf 
...
static void
ngx_http_wait_request_handler(ngx_event_t *rev) // 0事件可重入开始  r生命周期开启
{
    u_char                    *p;
    size_t                     size;
    ssize_t                    n;
...
ngx_http_alloc_request(ngx_connection_t *c)
{
    ...
    r->header_in = hc->busy ? hc->busy->buf : c->buffer; // 把hc/c中的数据偷走了
...
ngx_http_ssl_handshake(ngx_event_t *rev)
{
    ...
    if (n == 1) {
        if (buf[0] & 0x80 /* SSLv2 */ || buf[0] == 0x16 /* SSLv3/TLSv1 */) {
            ...
            if (ngx_ssl_create_connection(&sscf->ssl, c, NGX_SSL_BUFFER)        // 参数1这个ssl是server块级别的(在解析阶段分配的ssl) 而非链接级别的
                != NGX_OK)
            {
                ngx_http_close_connection(c);
                return;
...
int
ngx_http_ssl_servername(ngx_ssl_conn_t *ssl_conn, int *ad, void *arg)       // 此回调 匹配相应的server块
{
    ngx_int_t                  rc;
    ngx_str_t                  host;
...
/*
ngx_ssl_get_server_name (c=0x7fffe717d310, pool=0x803cc0, s=0x7fffffffcda0) at src/event/ngx_event_openssl.c:4467
ngx_http_ssl_variable (r=0x803d10, v=0x804990, data=4581347) at src/http/modules/ngx_http_ssl_module.c:501
ngx_http_get_indexed_variable (r=0x803d10, index=11) at src/http/ngx_http_variables.c:618
ngx_http_script_copy_var_len_code (e=0x7fffffffce50) at src/http/ngx_http_script.c:845
ngx_http_complex_value (r=0x803d10, val=0x7e8110, value=0x7fffffffcf00) at src/http/ngx_http_script.c:83
ngx_http_map_variable (r=0x803d10, v=0x8049a0, data=8290528) at src/http/modules/ngx_http_map_module.c:120
ngx_http_get_indexed_variable (r=0x803d10, index=12) at src/http/ngx_http_variables.c:618
ngx_http_script_copy_var_len_code (e=0x7fffffffcfc0) at src/http/ngx_http_script.c:845
ngx_http_complex_value (r=0x803d10, val=0x7ecbb8, value=0x7fffffffd060) at src/http/ngx_http_script.c:83
...
ngx_http_ssl_certificate(ngx_ssl_conn_t *ssl_conn, void *arg)
{
    ...
    for (i = 0; i < nelts; i++) { // nginx.conf.sni 此nelts为1

        if (ngx_http_complex_value(r, &certs[i], &cert) != NGX_OK) { // 传出的cert为 请求的域名对应的server<1|2>.crt // 底层逻辑应该是在某个位置 根据servername设置了variable
            goto failed;
        }
...
static void
ngx_http_process_request_line(ngx_event_t *rev) // 1事件可重入 解析第一行
{
    ssize_t              n;
    ngx_int_t            rc, rv;
    ngx_str_t            host;
...
static void
ngx_http_process_request_headers(ngx_event_t *rev) // 2事件可重入 解析header
{
    u_char                     *p;
    size_t                      len;
    ssize_t                     n;
...
ngx_http_process_request(ngx_http_request_t *r)
{
    ...
    ngx_http_handler(r);    // 3(解析完毕)事件可重入结束 走上层handler 事件的handler一律为ngx_http_request_handler  在其(ngx_http_request_handler)内部 根据读写再做处理(read/write_event_handler)
                            // 如果是get请求 那么c方向的read已经结束了 如果正在走上层的handler 那么此时c的读事件到来怎么办?
                            //       上层走block_reading 

                            //    |---read->handler = ngx_http_request_handler ---> r->read_event_handler(r);
                            // ev->
                            //    |---write->handler =  ngx_http_request_handler ---> r->write_event_handler(r);
}
...
static void
ngx_http_set_keepalive(ngx_http_request_t *r) // 业务完毕 清空r 重置c 设置长连接回调
{
    int                        tcp_nodelay;
    ngx_buf_t                 *b, *f;
    ngx_chain_t               *cl, *ln;
    ...
    ngx_http_free_request(r, 0); // 1清空r的pool 置位connection=destroyed

    c->data = hc;                   // 2恢复到hc // 为何抽象出一个hc结构体 为了预读取 为了更好的获取配置信息

    if (ngx_handle_read_event(rev, 0) != NGX_OK) { // 3只监听读
        ngx_http_close_connection(c);
        return;
    }
    ...
    if (b->pos < b->last) { // 一波未平(其实已平) 一波又起
    ...
    rev->handler = ngx_http_keepalive_handler; // 3 设置读回调
    ...
    c->idle = 1;
    ngx_reusable_connection(c, 1);  // 4挂到全局reusable链表上
...
static void
ngx_http_keepalive_handler(ngx_event_t *rev) // 长连接读事件到来
{
    size_t             size;
    ssize_t            n;
    ngx_buf_t         *b;
