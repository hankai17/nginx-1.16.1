...
static void
ngx_http_upstream_handler(ngx_event_t *ev) // UPSTREAM osvc的事件回调
{
    ngx_connection_t     *c;
    ngx_http_request_t   *r;
    ngx_http_upstream_t  *u;
...
static void
ngx_http_upstream_ssl_init_connection(ngx_http_request_t *r,    // "建链"(非建链成功)后调用 见上面这个函数 
                                                                // 场景1:新链接 场景2: 建连后或从池里拿到一个链接 
    ngx_http_upstream_t *u, ngx_connection_t *c)
{
    ngx_int_t                  rc;
    ngx_http_core_loc_conf_t  *clcf;
    ...
    if (u->conf->ssl_session_reuse) {
        ...
        if (u->peer.set_session(&u->peer, u->peer.data) != NGX_OK) {            // 第一次调用时候session为空 // 如果从池里拿到一个链接那么就用这个链接的session ?
                                                                                // https://github.com/alibaba/tengine/issues/1016
            ngx_http_upstream_finalize_request(r, u,
                                               NGX_HTTP_INTERNAL_SERVER_ERROR);
            return;
        }
    ...
    ngx_http_upstream_t *u, ngx_uint_t do_write)
{
    ...
    if (!u->request_sent) {
        ...
        r->read_event_handler = ngx_http_upstream_read_request_handler; // 设置c的读回调 确保c的请求 分多次到来 进行可重入的发送
    ...
    for ( ;; ) {
        ...
        if (r->reading_body) {
            ...
            rc = ngx_http_read_unbuffered_request_body(r); // POST流程 在proxy中形成的tunnel // 当然GET也复用了该流程
    ...
    if (!r->reading_body) {
        if (!u->store && !r->post_action && !u->conf->ignore_client_abort) {
            r->read_event_handler =
                                  ngx_http_upstream_rd_check_broken_connection; // 请求发送完毕 监控c的fin
        }
    }
...
ngx_http_upstream_process_header(ngx_http_request_t *r, ngx_http_upstream_t *u) // 6 UPSTREAM 读到OS响应后 开始构建第二个tunnel // 1读os header
{
    ...
    ngx_http_upstream_send_response(r, u); // 2发送os 响应头给vc
}
...
static void
ngx_http_upstream_send_response(ngx_http_request_t *r, ngx_http_upstream_t *u)  // 2发送os 响应头给vc
{
    ssize_t                    n;
    ngx_int_t                  rc;
    ngx_event_pipe_t          *p;
    ...
    rc = ngx_http_send_header(r); // 2.1走 ngx_http_top_header_filter
    ...
    p->temp_file = ngx_pcalloc(r->pool, sizeof(ngx_temp_file_t)); // 保存文件里
    if (p->temp_file == NULL) {
        ngx_http_upstream_finalize_request(r, u, NGX_ERROR);
        return;
    }
...
为何需要拷贝? 
    减少耦合 上层只需要把buf传过来即可 
为何需要深拷贝? 
    "网络原因"每次读不可能一下读满整个缓冲区 必须通过深拷贝来反应每块被割裂的buf块
    "割裂"一个独立的buf: sub中一块buf可能分好几片发出去
    
为何需要浅拷贝? 
*/

static void
ngx_http_upstream_process_non_buffered_request(ngx_http_request_t *r, // 什么时候用深拷贝? 什么时候用浅拷贝?
    ngx_uint_t do_write)
{
    size_t                     size;
    ssize_t                    n;
    ...
    for ( ;; ) { // 网络状况不好的情况下  u->buffer会被切成 小块的chain // 但是注意 pl 只会往后走 直到用完
                 // 所以怎么 一句话描述这个机制?

        if (do_write) {

            if (u->out_bufs || u->busy_bufs || downstream->buffered) {
                rc = ngx_http_output_filter(r, u->out_bufs); // 虽然原始的u->buffer早已经移动完毕 但底层的write_filter返回out_bufs时 反应的是底层发送了多少 所以需要掉update_chains清理
        ...
        if (size && upstream->read->ready) {
            ...
            n = upstream->recv(upstream, b->last, size); // 其实就是 各指针在一个u->buffer上移动 直到发送完毕才会读数据
...
static ngx_int_t
ngx_http_upstream_non_buffered_filter(void *data, ssize_t bytes)    // 感觉有点儿怪  虽然构造出了一个chain 指向同一个buffer 但是上层的buffer已经pos=last了(切断联系?) // 深拷贝概念: 即不仅仅重新构造了chain 而且构造/分配了chain里的buffer
{
...
static void
ngx_http_upstream_process_downstream(ngx_http_request_t *r) // vc得写回调 ngx_event_pipe(p, 1<doWrite>)  // 跟下边是一对儿
{
    ngx_event_t          *wev;
    ngx_connection_t     *c;
    ngx_event_pipe_t     *p;
...
static void
ngx_http_upstream_process_upstream(ngx_http_request_t *r, // upstream的读回调 里面调用ngx_event_pipe(p, 0) // 跟上边是一对儿
    ngx_http_upstream_t *u)
{
    ngx_event_t       *rev;
    ngx_event_pipe_t  *p;
...
static ngx_int_t
ngx_http_upstream_set_local(ngx_http_request_t *r, ngx_http_upstream_t *u, // 透明代理设置 bind特殊的客户端ip 不需要端口
...
ngx_http_upstream_init_main_conf(ngx_conf_t *cf, void *conf)
{
    ...
    for (i = 0; i < umcf->upstreams.nelts; i++) {
        ...
        init = uscfp[i]->peer.init_upstream ? uscfp[i]->peer.init_upstream:         // 如果有自定义的
                                            ngx_http_upstream_init_round_robin;     // 如果没有自定义的 ngx_http_upstream_init_keepalive
