...
ngx_http_block(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    *cf = pcf;
    ...
    if (ngx_http_optimize_servers(cf, cmcf, cmcf->ports) != NGX_OK) { // cmcf中除了有servers[] 还有ports[]
        return NGX_CONF_ERROR;
    }
    ...
    ngx_array_t *ports)
{
    ...
    for (p = 0; p < ports->nelts; p++) {
        ...
        if (ngx_http_init_listening(cf, &port[p]) != NGX_OK) { // 设置lfd回调
            return NGX_ERROR;
        }
    }
...
/*
port机制:
    ngx_http_core_listen 即解析server块的listen时构造lsopt(端口 地址 等属性) 然后调用ngx_http_add_listen/addresss/server 最终构造port结构  挂cmcf->ports数组上
    130行处: ngx_http_core_main_conf_t(简称为cmcf 里面有servers动态数组) 里面也有一个成员ports <ngx_http_conf_port_t>数组用来记录全局起的端口信息
    例 ports: <family, 80, addrs> <family, 8080, addrs>
                              |
...
ngx_http_add_listening(ngx_conf_t *cf, ngx_http_conf_addr_t *addr) // 针对某端口的其中一个地址 分配ls结构 包括设置lfd各种属性 ls->handler
{
    ...
    ls = ngx_create_listening(cf, addr->opt.sockaddr, addr->opt.socklen); // 将ls挂到cf->cycle->listening上  // 最终在子进程(ngx_event_process_init)中 挂树上监听
    if (ls == NULL) {
        return NULL;
    }
...
#endif
    ls->transparent = addr->opt.transparent; // 将地址上的透明属性 设置给ls
