...
ngx_event_pipe_read_upstream(ngx_event_pipe_t *p)
{
    ...
    for ( ;; ) {
        ...
        if (p->preread_bufs) {
            ...
            if (p->limit_rate) {    // 为什么在读os得方向上限速(从源头限速)? 标准tunnel则是在发送给vc时限速
            ...
            if (p->free_raw_bufs) { // 决定 从哪分配b // 在将8个buff发出去后 p->allocated并不会-- 第二次读os时 free_raw_bufs非空 而且是8 + 1个buffer 所以第二次一下就读了36k
            ...
            } else if (p->allocated < p->bufs.num) { // 第一次首先来这里 // 8个

                /* allocate a new buf if it's still allowed */

                b = ngx_create_temp_buf(p->pool, p->bufs.size); // 4K
                if (b == NULL) {
                    return NGX_ABORT;
                }
            ...
            } else if (!p->cacheable                // 读到8个buf后走这
                       && p->downstream->data == p->output_ctx
                       ...
                       && !p->downstream->write->delayed)
            {
                ...
                break;
                       ...
                       || p->temp_file->offset < p->max_temp_file_size) // allocate buf====>|copy_filter|====>p->in====>|发送前|====>out====>|发送后update|====>free_raw_bufs
            {
                ...
                rc = ngx_event_pipe_write_chain_to_temp_file(p); // 确实都憋在p->in里的
        ...
        while (cl && n > 0) {
            ...
            if (n >= size) { // 读os满整个buf
                ...
                if (p->input_filter(p, cl->buf) == NGX_ERROR) { // ngx_http_proxy_copy_filter 深拷贝并挂到p->in队列上
                    return NGX_ABORT;
                }
                ...
                ln = cl;
                cl = cl->next;
                ngx_free_chain(p->pool, ln); // 此后allocate中分配的b 变成孤独的b了 只存在与p->in链表中的shadow中
    ...
    // 1 8个buffer累积尽了 且 vc是write是ready的
...
#endif
    // 1 8个buffer累积尽了 且 vc是write是ready的 下面会置为do_write=1 直接返回到ngx_event_pipe中
    ...
    if (p->free_raw_bufs && p->length != -1) {
        ...
        if (cl->buf->last - cl->buf->pos >= p->length) {
            ...
            if (p->input_filter(p, cl->buf) == NGX_ERROR) {  // !!!!!!!!!!! 最终整个chain把数据放哪了?
                return NGX_ABORT;
            }
...
ngx_event_pipe_write_to_downstream(ngx_event_pipe_t *p)
{
    ...
    for ( ;; ) {
        ...
        for ( ;; ) {                        // 将p->in中的buffer串 挂out上 // 设计理念是 取出p->in中的原始buffer挂到out上 如果out太长那么就flush
            if (p->out) {                   // p->out证明写盘了 p->out记录了写盘位置(ngx_event_pipe_write_chain_to_temp_file) 从磁盘中拿老数据  // 从磁盘拿数据竟没有阈值限制
                cl = p->out;
                ...
                if (cl->buf->recycled && prev_last_shadow) {
                    if (bsize + cl->buf->end - cl->buf->start > p->busy_size) { // 如果out上的buf太多了 超过了p->busy_size 8192 // 即out上最多挂两个buf
                        flush = 1;
                        break;
                    }
    ...
    flush:  // out上满了即大于buy_size 8192 或 从磁盘上取得数据
        ...
        if (out == NULL) {
            ...
            if (flushed++ > 10) {
                return NGX_BUSY;
                                                   ...
                                                   // 此时的场景是: 有8个孤独的b
                                                   //              p->in即 深拷贝了这8个b 组成一串chain 且shadow指向8个孤独的b
                                                   //              out就是从原始的p->in上摘下来两个
                                                   //              现在将out发出去
        rc = p->output_filter(p->output_ctx, out); // ngx_http_upstream_output_filter //把串起来的tmp_out发出去
        ...
        for (cl = p->free; cl; cl = cl->next) { // 将发送出去的chain 生产到free_raw_bufs

            if (cl->buf->temp_file) {
                if (p->cacheable || !p->cyclic_temp_file) { // 忽略磁盘文件buf
                    continue;
                }
...
ngx_event_pipe_write_chain_to_temp_file(ngx_event_pipe_t *p)
{
    ...
    if (!p->cacheable) {
        ...
        do {
            ...
            if (prev_last_shadow
                && ((size + bsize > p->temp_file_write_size) // 跟发送一样 这里也有个阈值 最多写磁盘8K
    ...
    n = ngx_write_chain_to_temp_file(p->temp_file, out);  // pwrite(out) 阻塞写完
...
free:
    ...
    for (cl = out; cl; cl = next) { // 挂到free_raw_bufs后边?
        next = cl->next;
...
ngx_event_pipe_add_free_buf(ngx_event_pipe_t *p, ngx_buf_t *b)
{
    ...
    if (p->free_raw_bufs->buf->pos == p->free_raw_bufs->buf->last) {
        ...
        p->free_raw_bufs = cl;                              // 挂到p->free_raw_bufs上
