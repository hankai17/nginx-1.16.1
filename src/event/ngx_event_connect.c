...
ngx_event_connect_peer(ngx_peer_connection_t *pc) // 与OS建联
{
    ...
    rc = pc->get(pc, pc->data);                     // 与os建联之前 先调用get函数 一个是upstream_keepalive模块 一个是upstream_rr模块(V 直接proxy_pass) 选哪个?
                                                    // 进行try统计
                                                    // 返回DONE即 说明找到了匹配的c 就不会起socket了
    if (rc != NGX_OK) {
        return rc;
    }
    ...
    s = ngx_socket(pc->sockaddr->sa_family, type, 0); // 起socket
    ...
    c = ngx_get_connection(s, pc->log); // 拿一个c
    ...
    if (pc->local) { // 透明代理时 配置local 需要bind 原客户端的ip/port
