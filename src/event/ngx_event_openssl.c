...
ngx_int_t
ngx_ssl_create(ngx_ssl_t *ssl, ngx_uint_t protocols, void *data)                    // 0创建SSL_CTX对象 
{
...
ngx_int_t
ngx_ssl_certificate(ngx_conf_t *cf, ngx_ssl_t *ssl, ngx_str_t *cert, // 分别为配置中的 公钥证书 私钥文件 // 分两大步骤 先解析 后加载 // 1将公钥私钥证书加载到ctx中
    ngx_str_t *key, ngx_array_t *passwords)
{
    char            *err;
    X509            *x509;
    ...
    x509 = ngx_ssl_load_certificate(cf->pool, &err, cert, &chain); // 只解析PEM格式的公钥证书 并插入证书链中 // 返回值x509为公钥证书的抽象结构体
    if (x509 == NULL) {
        if (err != NULL) {
            ngx_ssl_error(NGX_LOG_EMERG, ssl->log, 0,
    ...
    if (SSL_CTX_use_certificate(ssl->ctx, x509) == 0) { // 加载公钥证书到 ssl->ctx中
        ngx_ssl_error(NGX_LOG_EMERG, ssl->log, 0,
    ...
    while (n--) {
        ...
        if (SSL_CTX_add_extra_chain_cert(ssl->ctx, x509) == 0) {            // 加载证书链上 额外的证书公钥 到ssl->ctx中
            ngx_ssl_error(NGX_LOG_EMERG, ssl->log, 0,
    ...
    sk_X509_free(chain);
                                                                            ...
                                                                            // 至此 证书公钥 加载到ssl->ctx完毕
    pkey = ngx_ssl_load_certificate_key(cf->pool, &err, key, passwords);    // 解析 PEM格式的私钥证书
    if (pkey == NULL) {
        if (err != NULL) {
            ngx_ssl_error(NGX_LOG_EMERG, ssl->log, 0,
    ...
    if (SSL_CTX_use_PrivateKey(ssl->ctx, pkey) == 0) {                      // 加载 私钥证书到 ssl->ctx中
        ngx_ssl_error(NGX_LOG_EMERG, ssl->log, 0,
    ...
    return NGX_OK;                                                          // 至此 证书公钥 证书私钥 都加载到了ssl->ctx中了
}
    ...
    STACK_OF(X509) **chain)
{
    ...
    *chain = sk_X509_new_null();
    ...
    for ( ;; ) {
        ...
        temp = PEM_read_bio_X509(bio, NULL, NULL, NULL); // PEM格式
        if (temp == NULL) {
        ...
        if (sk_X509_push(*chain, temp) == 0) { // PEM解析成功放到chain里
    ...
    ngx_str_t *key, ngx_array_t *passwords)
{
    ...
    for ( ;; ) {
        ...
        pkey = PEM_read_bio_PrivateKey(bio, NULL, cb, pwd); // 加载 PEM格式的私钥证书
        if (pkey != NULL) {
            break;
        }
...
ngx_ssl_check_host(ngx_connection_t *c, ngx_str_t *name)
{
    ...
    cert = SSL_get_peer_certificate(c->ssl->connection);    // 获取客户端证书 // 验证证书合法性?
    if (cert == NULL) {
        return NGX_ERROR;
    }
