    ...
    ngx_log_t *log)
{
    ...
    ngx_http_lua_inject_socket_tcp_api(log, L); // 插入 socket.
    ...
    ngx_http_lua_ctx_t *ctx, volatile int nrets)
{
    ...
    NGX_LUA_EXCEPTION_TRY {
        ...
        for ( ;; ) {    // openresty的核心仍然是基于nginx epoll的event驱动(nginx的es是驱动源) 而非基于一般协程架构的全局队列
                        // 这里for循环什么? 遍历这个http事务上的所有协程(\/)
            ...
            rv = lua_resume(orig_coctx->co, nrets);             // 原理是什么? 保存当前上下文到L 然后... 非对称协程?
            ...
            case LUA_YIELD:
                 ...
                 * lua_yield()
                 */
                switch(ctx->co_op) {                                            // yeild原因
                case NGX_HTTP_LUA_USER_CORO_NOP:                                // 1)ngx.socket读写 ngx.sleep 直接返回again交给es处理
                ...
                case NGX_HTTP_LUA_USER_THREAD_RESUME:                           // 2)ngx.thread.spawn(见ngx_http_lua_uthread.c: ngx_http_lua_uthread_spawn)
                    ...
                    break;                                                      //      break 出来继续走 即调用新协程

                case NGX_HTTP_LUA_USER_CORO_RESUME:                             // 3)子协程里 调用coroutine.resume // https://my.oschina.net/u/2539854/blog/853012
                    ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
