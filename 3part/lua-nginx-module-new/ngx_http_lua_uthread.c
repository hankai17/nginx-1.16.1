...
ngx_http_lua_uthread_spawn(lua_State *L)                            // 起协程 老协程挂到队列上 然后主动yeild到上层调度层调度之
{
    ...
    ngx_http_lua_coroutine_create_helper(L, r, ctx, &coctx);            // 新协程coctx
    ...
    ctx->co_op = NGX_HTTP_LUA_USER_THREAD_RESUME;                       // 设置协程挂起原因

    ctx->cur_co_ctx->thread_spawn_yielded = 1;

    if (ngx_http_lua_post_thread(r, ctx, ctx->cur_co_ctx) != NGX_OK) {  // 将当前协程(access/content阶段自动起的协程co/L)挂到队列上 相当于mysylar中的yield
        return luaL_error(L, "no memory");
    }

    coctx->parent_co_ctx = ctx->cur_co_ctx;                             // 设置新协程coctx的父亲为当前协程(access/content阶段自动起的协程co/L) 并将新协程coctx设置为当前协程
    ...
    return lua_yield(L, 1);                                             // 到上层调度层 立即执行新协程coctx
}
