...
ngx_http_lua_init_registry(lua_State *L, ngx_log_t *log)
{
    ...
    lua_pushlightuserdata(L, ngx_http_lua_lightudata_mask(      // ngx_http_lua_ctx_tables 就是在这里注册到 Nginx 内存的
                          socket_pool_key));
    ...
    ngx_http_lua_main_conf_t *lmcf, ngx_log_t *log)
{
    ...
    ngx_http_lua_inject_ngx_api(L, lmcf, log); // 注册ngx lua函数
}


static void
ngx_http_lua_inject_ngx_api(lua_State *L, ngx_http_lua_main_conf_t *lmcf,
    ngx_log_t *log)         // 注册各种 Nginx 层面的 API 函数，设置字符串 ngx 为表名，lua 代码中就可以使用 ngx.* 来调用这些 API 了。
{
