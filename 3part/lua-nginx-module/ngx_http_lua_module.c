...
static ngx_int_t
ngx_http_lua_init(ngx_conf_t *cf)   // 注册handler filter //创建lua vm相关 // master fork worker进程时，Lua 虚拟机自然也被复制（COW）到了 worker 进程。
{
...
#   endif
        ...
        rc = ngx_http_lua_init_vm(&lmcf->lua, NULL, cf->cycle, cf->pool,    // 创建和初始化一个lua虚拟机环境
                                                                            // 由lua API luaL_newstate实现 该接口函数会创建一个协程作为主协程 返回 lua_state（存放堆栈信息，包括后续的请求数据（ngx_http_request_t ），API 注册表等数据都是存放在这里，供 lua 层使用）
                                  lmcf, cf->log, NULL);
        if (rc != NGX_OK) {
            if (rc == NGX_DECLINED) {
