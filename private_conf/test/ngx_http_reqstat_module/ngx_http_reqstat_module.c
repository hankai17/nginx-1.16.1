...
static ngx_http_module_t  ngx_http_reqstat_module_ctx = {
    ngx_http_reqstat_add_variable,         /* preconfiguration */           // 调用时机是提前布局后 确保其下标是0
    ...
    ngx_http_reqstat_create_loc_conf,      /* create location configuration */  // http块内的upstream块也会调用
...
static void *
ngx_http_reqstat_create_main_conf(ngx_conf_t *cf) // 只能在http块 所以只调用了一次
{
...
ngx_http_reqstat_add_variable(ngx_conf_t *cf)
{
    ...
    var = ngx_http_add_variable(cf, &name, 0);              // 把reqstat_enable 插入core main config的variables中
    if (var == NULL) {
        return NGX_ERROR;
    }
    ...
    rmcf->index = ngx_http_get_variable_index(cf, &name);   // 记录下表
...
static char *
ngx_http_reqstat_zone(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)       // 设置共享内存大小 将key设置到共享内存中 // 此关键字不依赖config // 分配ctx ctx由shm管理
{
    ssize_t                            size;
    ngx_str_t                         *value;
    ngx_shm_zone_t                    *shm_zone;
    ...
    ccv.cf = cf;
    ccv.value = &value[2]; // $host,$server_addr:$server_port
...
ngx_http_reqstat(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    ngx_http_reqstat_conf_t      *rlcf = conf;                              // 配置写在哪 就是哪的local槽里的config

    value = cf->args->elts;
    smcf = ngx_http_conf_get_module_main_conf(cf, ngx_http_reqstat_module); // main config只有一个 无论在哪个块里调用该函数 均一样
    ...
    for (i = 1; i < cf->args->nelts; i++) {
        ...
        z = ngx_array_push(rlcf->monitor);                                  // 把要监控的共享内存挂 config的监控队列里
        *z = shm_zone;
        ...
        if (j == smcf->monitor->nelts) {                                    // 如果main中没有监控 则加上去
...
static ngx_int_t
ngx_http_reqstat_variable_enable(ngx_http_request_t *r,                     // 根据request的ctx的bypass标志位 重置variable 
    ngx_http_variable_value_t *v, uintptr_t data)
{
    u_char                       *p;
...
ngx_http_reqstat_init_handler(ngx_http_request_t *r)
{
    ...
    if (store == NULL) {
        ...
        store = ngx_http_reqstat_create_store(r, rlcf); // 分配store // 计算key 分配红黑树node 并插入红黑树
        if (store == NULL) {
            return NGX_ERROR;
        }
    ...
    if (ngx_http_get_indexed_variable(r, rmcf->index) == NULL) { // ngx_http_reqstat_variable_enable
        return NGX_ERROR;
    }
...
ngx_http_reqstat_log_handler(ngx_http_request_t *r)
{
    ...
    for (i = 0; i < store->monitor_index.nelts; i++) {  // 遍历所有监控的共享内存 // 更新各共享内存中的数据
...
ngx_http_reqstat_rbtree_lookup(ngx_shm_zone_t *shm_zone, ngx_str_t *val)
{
    ...
    ngx_memcpy(rs->data, val->data, len);   // 0.0.0.0,127.0.0.1:9527
    rs->len = len;
...
static ngx_http_reqstat_store_t *
ngx_http_reqstat_create_store(ngx_http_request_t *r,        // 计算key 并将其插入相应的共享内存红黑树中 分配一个store并记录这个红黑树节点
    ngx_http_reqstat_conf_t *rlcf)
{
    ngx_str_t                     val, *value;
    ngx_uint_t                    i;
    ...
    for (i = 0; i < rlcf->monitor->nelts; i++) {    // 遍历监控对象
        z = shm_zone[i];
        ctx = z->data;                              // ctx中有key: $host,$server_addr:$server_port

        if (ngx_http_complex_value(r, &ctx->value, &val) != NGX_OK) { // 根据key拿到val: 0.0.0.0,127.0.0.1:9527
            ngx_log_error(NGX_LOG_WARN, r->connection->log, 0,
                          "failed to reap the key \"%V\"", ctx->val);
            continue;
        }
        ...
        *value = val;
        ...
        fnode = ngx_http_reqstat_rbtree_lookup(shm_zone[i], &val);      // 插入语义 将val抽象成rs插入红黑树
        if (fnode == NULL) {
            ngx_log_error(NGX_LOG_WARN, r->connection->log, 0,
                          "failed to alloc node in zone \"%V\", "
                          "enlarge it please",
