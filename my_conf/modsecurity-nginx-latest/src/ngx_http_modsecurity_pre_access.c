...
ngx_http_modsecurity_request_read(ngx_http_request_t *r)
{
    ...
    if (ctx->waiting_more_body)
    {
        ...
        ngx_http_core_run_phases(r); // post上传完整文件后重新进入ngx_http_modsecurity_pre_access_handler
    }
}
...
ngx_int_t ngx_http_modsecurity_pre_access_handler(ngx_http_request_t *r)
{
    ...
    if (m_ctx->waiting_more_body == 0)
    {
        ...
        task = ngx_thread_task_alloc(r->pool, sizeof(ngx_http_modsecurity_pre_access_thread_ctx_t)); // 非侵入性设计 

        ctx = task->ctx; // 将可调对象+参数封装到task里
        ctx->r = r;
        ctx->ctx = m_ctx;
