...
static ngx_int_t
ngx_http_upstream_session_sticky_init_peer(ngx_http_request_t *r,           // 建联前夕调用 4UPSTREAM 初始化pc的get/free/try    // hankai2
    ngx_http_upstream_srv_conf_t *us)
{
    ngx_int_t                          rc;
    ...
    sspd = ngx_pcalloc(r->pool, sizeof(ngx_http_upstream_ss_peer_data_t));  // hankai2.0 分配sspd
    if (sspd == NULL) {
        return NGX_ERROR;
    }

    r->upstream->peer.data = &sspd->rrp;                                    
    rc = ngx_http_upstream_init_round_robin_peer(r, us);                    // hankai2.1 初始化rrp(从upstream中拷贝ip等信息) 初始化pc的get/free/try
    if (rc != NGX_OK) {
        return rc;
    }
    ...
    ctx = ngx_http_get_module_ctx(r, ngx_http_upstream_session_sticky_module);  // hankai2.2 分配ctx
    if (ctx == NULL) {
    ...
    r->upstream->peer.get = ngx_http_upstream_session_sticky_get_peer;      // 覆盖上面rr中设置的get
    ...
    void *conf)
{
    ...
    uscf->peer.init_upstream = ngx_http_upstream_session_sticky_init_upstream;      // 重置upstream的init_upstream     此函数在init_main_conf中调用 // hankai1
                                                                                    // rr是默认的 ss keepalive都是额外的 如果没有ss keepalive则调用rr

    uscf->flags = NGX_HTTP_UPSTREAM_CREATE                                          // ss配置主要存放于sscf中
    ...
    ngx_http_upstream_srv_conf_t *us)
{
    ...
    if (ngx_http_upstream_init_round_robin(cf, us) != NGX_OK) {                 // 提取us中写的ip集合 存放到rr的peers中
        return NGX_ERROR;
    }
    ...
    for (peer = peers->peer, i = 0; peer; peer = peer->next, i++) {             // 把rr中的peers再拷贝到sscf中
    ...
    us->peer.init = ngx_http_upstream_session_sticky_init_peer;                 // 建联前夕调用 4UPSTREAM 初始化pc的get/free/try // hankai2
