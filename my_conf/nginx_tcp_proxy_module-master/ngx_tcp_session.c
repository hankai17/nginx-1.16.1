...
void
ngx_tcp_init_connection(ngx_connection_t *c) // 0STREAM开始 lfd得回调
{
    ngx_uint_t            i;
    ngx_tcp_port_t       *port;
    struct sockaddr      *sa;
    ...
    s = ngx_pcalloc(c->pool, sizeof(ngx_tcp_session_t));    // 0STREAM 分配session
    if (s == NULL) {
        ngx_tcp_close_connection(c);
        return;
    }
    ...
    ngx_tcp_init_session(c);        // 1STREAM 初始化sesseion
}
...
ngx_tcp_init_session(ngx_connection_t *c)
{
    ...
    ngx_tcp_set_session_socket(s);  // 2STREAM 设置session socket属性等
...
ngx_tcp_process_session(ngx_connection_t *c)
{
    ...
    cscf->protocol->init_session(s); // 2STREAM 初始化session本体 (多态)
}
