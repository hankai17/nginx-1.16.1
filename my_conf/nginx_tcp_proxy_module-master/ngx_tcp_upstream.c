...
void
ngx_tcp_upstream_init(ngx_tcp_session_t *s)     // 2.2STREAM 与OS建联
{
    ngx_str_t                      *host;
    ngx_uint_t                      i;
    ngx_connection_t               *c;
...
ngx_tcp_upstream_connect(ngx_tcp_session_t *s, ngx_tcp_upstream_t *u) 
{
    ...
    c->write->handler = ngx_tcp_upstream_handler;       // 2.2STREAM 与os建联后 重置osvc事件回调
...
static void
ngx_tcp_upstream_handler(ngx_event_t *ev)  // 为何不定义c端得handler? 跟ats一样 ats中也没有
{
    ngx_connection_t     *c;
    ngx_tcp_session_t    *s;
    ngx_tcp_upstream_t   *u;
