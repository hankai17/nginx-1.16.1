...
ngx_module_t  ngx_tcp_proxy_module = { // 1.定义全局module
    NGX_MODULE_V1,
    &ngx_tcp_proxy_module_ctx,             /* module context */
    ngx_tcp_proxy_commands,                /* module directives */
    NGX_TCP_MODULE,                        /* module type */  // 2.设置module为tcp类型
    NULL,                                  /* init master */
    NULL,                                  /* init module */
    NULL,                                  /* init process */
    NULL,                                  /* init thread */
...
static void 
ngx_tcp_proxy_init_session(ngx_tcp_session_t *s) // 2STREAM 初始化session本体 (多态)
{
    ngx_connection_t         *c;
    ...
    ngx_tcp_proxy_init_upstream(c, s);  // 2.1STREAM 初始化session得upstream
...
static  void
ngx_tcp_proxy_init_upstream(ngx_connection_t *c/*客户端得c*/, ngx_tcp_session_t *s)
{
    ngx_tcp_upstream_t       *u;
    ...
    u->write_event_handler = ngx_tcp_upstream_init_proxy_handler;   // 2.1STREAM 设置u->c业务回调
    ...
    ngx_tcp_upstream_init(s); //2.2STREAM // 里面与os建联 定义在tcp_upstream模块中(也就是说要实现一个tcp upstream中间层)
...
static void 
ngx_tcp_upstream_init_proxy_handler(ngx_tcp_session_t *s, ngx_tcp_upstream_t *u) // 3STREAM osvc 第一次事件到来
{
    ngx_connection_t         *c;
    ...
    c = pctx->upstream->connection; // osvc
    ...
    s->connection->read->handler = ngx_tcp_proxy_handler;   // 3STREAM 链接上了 重置vc/osvc端事件回调
...
static void
ngx_tcp_proxy_handler(ngx_event_t *ev)      // 4STREAM vc/osvc事件回调  // 里面建立tunnel
{
